...
class StandardTrainer:
    ...
    def trainWithBatch(self, imagebatch, targetbatch):
        ...
        now = datetime.now()
        print('time before network pass:', )
        output = self.net(imagebatch)
        later = datetime.now()
        print('time taken for passing through network:', later-now)
    ...
    def trainWithLoader(self, torchloader, epochs, testloader=None, maxBatchesPerEpoch=None, testinterval=5):
        ...
        for e in range(0, epochs):
            ...
            num_items = 0
            now = datetime.now()
            print('time before getting batch from data loader:', )
            for i_batch, sample_batched in enumerate(torchloader):
                later = datetime.now()
                print('time taken for getting batch from data loader:', later - now)

...
def set_patch_size(config):
    ...
    # in the case we're using the Lits data we need to be aware that resampling factor shouldn't be taken into account
    # as it is always one due tot he fact that the resampled data is provided instead of doing resampling on the fly
    # as was the case with BTD
...
#
#     if dataset == "BTD":
#         new_patch_size = [(i // 16) * 16 if i < 112 else 112 for i in min_size]
#
#     if dataset == "LitsData":
#         res_factor = int(data_loc.split(os.sep)[-1].split("Res")[-1])
#         min_img_size = [i if i >= 112 / res_factor else int(112 / res_factor) for i in min_size]
#         new_patch_size = [(i // 16) * 16 if i < 112 else 112 for index, i in enumerate(min_img_size)]
#
#     print('Setting patch size from:', config['Patch size'], 'to:', new_patch_size)
#     return patch_size

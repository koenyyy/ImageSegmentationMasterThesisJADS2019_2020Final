...
class UNet3D(nn.Module):
    def __init__(self, k=1, channels_in=1, channels_out=2, kernel_size=3,
        ...
        self.pool1 = nn.MaxPool3d(self.pooling_kernel, self.pooling_stride)
        self.pool2 = nn.MaxPool3d(self.pooling_kernel, self.pooling_stride)
        self.pool3 = nn.MaxPool3d(self.pooling_kernel, self.pooling_stride)
        self.pool4 = nn.MaxPool3d(self.pooling_kernel, self.pooling_stride)
        ...
        self.UNetConvBlock1 = UNetConvBlock3D(self.channels_in, 1 * self.k, self.kernel_size,
                                              self.activation, self.dropout, self.batchnorm, self.instancenorm,
                                              self.padding)
        self.UNetConvBlock2 = UNetConvBlock3D(1*self.k, 2 * self.k, self.kernel_size,
                                              self.activation, self.dropout, self.batchnorm, self.instancenorm,
                                              self.padding)
        self.UNetConvBlock3 = UNetConvBlock3D(2 * self.k, 4 * self.k, self.kernel_size,
                                              self.activation, self.dropout, self.batchnorm, self.instancenorm,
                                              self.padding)
        self.UNetConvBlock4 = UNetConvBlock3D(4 * self.k, 8 * self.k, self.kernel_size,
                                              self.activation, self.dropout, self.batchnorm, self.instancenorm,
                                              self.padding)
        self.UNetConvBlock5 = UNetConvBlock3D(8 * self.k, 16 * self.k, self.kernel_size,
                                              self.activation, self.dropout, self.batchnorm, self.instancenorm,
                                              self.padding)
        self.UNetConvBlock6 = UNetConvBlock3D(16 * self.k, 8 * self.k, self.kernel_size,
                                              self.activation, self.dropout, self.batchnorm, self.instancenorm,
                                              self.padding)
        self.UNetConvBlock7 = UNetConvBlock3D(8 * self.k, 4 * self.k, self.kernel_size,
                                              self.activation, self.dropout, self.batchnorm, self.instancenorm,
                                              self.padding)
        self.UNetConvBlock8 = UNetConvBlock3D(4 * self.k, 2 * self.k, self.kernel_size,
                                              self.activation, self.dropout, self.batchnorm, self.instancenorm,
                                              self.padding)
        self.UNetConvBlock9 = UNetConvBlock3D(2 * self.k, 1 * self.k, self.kernel_size,
                                              self.activation, self.dropout, self.batchnorm, self.instancenorm,
                                              self.padding)

        self.finalConv = nn.Conv3d(1 * self.k, self.channels_out, 1, )
    ...
    def forward(self, x):
        ...
        # oke alles is kut maar gelukkig ben je een slimme jonge koen. Je moet even kijken of je data een even aantal slices heeft, zo niet dan wordt ie een keer oneven na maxpooling
        skip3 = res.clone()
        res = self.pool3(res)
...
class UNetConvBlock3D(nn.Module):
    ...
    def __init__(self, channels_in, channels_out, kernel_size, activation=F.leaky_relu, dropout=False,
        ...
        self.activation = activation
        self.conv1 = nn.Conv3d(channels_in, channels_out, kernel_size, padding=padding)
        self.conv2 = nn.Conv3d(channels_out, channels_out, kernel_size, padding=padding)
...
class UpConv(nn.Module):
    ...
    def __init__(self, channels_in, channels_out, kernel_size, stride, padding=True):
        ...
        self.upsample = nn.ConvTranspose3d(channels_in, channels_out, kernel_size, stride)
